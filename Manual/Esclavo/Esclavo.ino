...
//  noInterrupts();
  ...
  while (salidaEncontrada == false) {
    ...
    if (DISTANCIA < 23) {
      ...
      if (cmLeft < 20 && cmRight < 20){ ///SI NO HAY SALIDA DEBE DE GIRAR 180 GRADOS
          caminoSinSalida++;
          turnAround(2);
          girosDerecha = girosDerecha + 2;
          Stop();     
      }else if (cmLeft > cmRight) { /// SI HAY SALIDA HACIA LA IZQUIERDA, HACE UN GIRO HACIA ESE LADO
        tiempoAhora = millis();
